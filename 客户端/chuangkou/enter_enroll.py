...
class Dengluzhuce:
    def __init__(self, sockfd, addr):
        ...
        self.login_info = ''
        # Unicode所有的中文
        self.zhongwen = {chr(x) for x in range(0x4e00, 0x9f9d)}
        self.conn = sockfd
        self.addr = addr
        self.zhanghaomima = ''
        # print('账号密码', self.zhongwen)
        self.retu = 1
        ...
        def gets_info():
            ...
            if newname:
                for x in newname:
                    if x == '@':
                        ...
                        # print('self.zhongwen',self.zhongwen)
            ...
            if len(not_data) != 0:
                ...
                if data.decode()[0] != 'Y':
                    ...
                    print(data.decode())
                    message.showinfo(title='注册用户成功了', message=data[2:].decode())
                    quitzhuce()
                # ********************************************
